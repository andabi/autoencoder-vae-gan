...
class Generator(object):
    def __init__(self, code_size=Z_SIZE):
        ...
        self.z = tf.placeholder(tf.float32, (None, code_size), name='z')
        self()
...
class Discriminator(object):
    ...
    def __call__(self, x=None):
        ...
        with tf.variable_scope('disc'):
            ...
            out_2 = fc_bn('out_2', out_1, H2_SIZE, is_training=self.is_training)
            out_3 = fc_bn('out_3', out_2, H3_SIZE, is_training=self.is_training)
            d = fc_bn('out', out_3, 1, act=tf.nn.sigmoid, is_training=self.is_training)
        return d

    def discriminate(self, sess, data):
        d = sess.run(self(), feed_dict={self.x: data, self.is_training: False})
        return d
...
class GD(object):
    ...
    def _loss_gen(self):
        loss = -1. * tf.reduce_mean(tf.log(self.disc(self.gen())))
        return loss
