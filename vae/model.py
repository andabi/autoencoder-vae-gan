...
class VariationalAutoEncoder(object):
    def __init__(self, input_size=X_SIZE, code_size=Z_SIZE, ckpt_path='checkpoints'):
        ...
        self.mu = self._encoder(self.input, code_size)
        self.code_size = code_size
        self.code = self._sample_code(self.mu, self.batch_size)
    ...
    def train(self, sess, data, final_step, lr, batch_size, ckpt_step=1):
        ...
        grad_loss = tf.gradients(self.loss, [self.w_mu])
        grad_encoder = tf.gradients([self.mu], [self.h2_mu])
        ...
        for step in range(global_step.eval(), final_step):
            ...
            if (step + 1) % ckpt_step == 0:
                ...
                print 'step-{}\td_loss={:2.2f}%\tloss={}\tgrad_loss={}'.format(step, d_loss * 100, loss, [np.linalg.norm(g) for g in g_loss])
    ...
    def generate(self, sess, visualizer, num=2):
        ...
        code = sess.run(self._sample_code(mu, num))
        print code
